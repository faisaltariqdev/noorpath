...
export async function loadSurahAyahs(surah: number): Promise<QuranAyah[]> {
  const { parasForSurah } = await import("./paras");
  const juzs = parasForSurah(surah);
  const files = await Promise.all(juzs.map((juz) => loadPara(juz)));
  return files.flatMap((file) => file.ayahs.filter((ayah) => ayah.surah === surah));
}
